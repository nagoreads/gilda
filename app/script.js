...
const URL_API_DIRECTA = `https://google.com{SHEET_ID}/gviz/tq?tqx=out:json&sheet=${SHEET_TAB_NAME}`;
...
async function cargarDatosDesdeSheets() {
  try {
    ...
    const usuariasMapeadas = filas.map(fila => {
      ...
      const celdaNombre = fila.c; 
      const celdaPagina = fila.c; 
...
function construirEdificio() {
  ...
  usuariasClub.forEach((usuaria, indice) => {
    ...
    contenedorVentana.classList.add('ventana-contenedor');
    ...
    // INTERACTIVO: Al pulsar, conmuta la luz y abre el bocadillo elástico
...
function mostrarHabitacionPropia(usuaria) {
  ...
  // Redacción limpia en una única línea recta
  ...
  contenedorHabitacion.style.display = "flex"; 
  if (btnCaptura) btnCaptura.style.display = "block"; 
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnCaptura) {
    btnCaptura.addEventListener('click', () => {
      document.body.classList.add('modo-screenshot');
    });
  }
